...
export const pythonDetector: Detector = {
  ...
  async detect(workspaceRoot: string): Promise<SuggestedAction[]> {
    ...
    if (foundFile === 'pyproject.toml') {
      actions.push(
        { id: 'python-pip-install', label: 'pip install -e .', command: 'pip install -e .', source: 'pyproject.toml' },
        { id: 'python-build', label: 'build', command: 'python -m build', source: 'pyproject.toml' },
        { id: 'python-pytest', label: 'pytest', command: 'pytest', source: 'pyproject.toml' },
      );
    } else if (foundFile === 'setup.py') {
      actions.push(
        { id: 'python-setup-build', label: 'build', command: 'python setup.py build', source: 'setup.py' },
        { id: 'python-setup-test', label: 'test', command: 'python setup.py test', source: 'setup.py' },
        { id: 'python-setup-install', label: 'install', command: 'python setup.py install', source: 'setup.py' },
      );
